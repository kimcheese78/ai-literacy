...
(function () {
  ...
  // --- Which-tool quiz ---
  (function () {
    var cfg = data('quiz-data');
    var live = document.getElementById('quiz-live');
    if (!cfg || !live) return;
    document.getElementById('quiz-static').hidden = true;
    live.hidden = false;
    ...
    var reason = document.getElementById('quiz-reason');

    function clearPick() {
      document.querySelectorAll('.card.recommended').forEach(function (c) { c.classList.remove('recommended'); });
    }

    function ask(nodeId) {
      clearPick();
      result.classList.remove('show');
      var node = cfg.nodes[nodeId];
      qEl.hidden = false;
      opts.hidden = false;
      ...
      node.opts.forEach(function (pair) {
        ...
        b.addEventListener('click', function () {
          if (cfg.nodes[pair[1]]) ask(pair[1]);
          else finish(pair[1]);
        });
        opts.appendChild(b);
      });
    ...
    function finish(key) {
      var res = cfg.results[key];
      qEl.hidden = true;
      opts.hidden = true;
      verdict.textContent = 'Start with ' + res.name + '.';
      reason.textContent = res.why;
      result.classList.add('show');
      var card = document.getElementById('tool-' + key);
      if (card) {
    ...
    document.getElementById('quiz-reset').addEventListener('click', function () { ask(cfg.start); });
    ask(cfg.start);
  })();
})();
